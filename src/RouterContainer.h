...
struct RouterNode {
    RouterNode(const std::shared_ptr<Router> &r, const double xPos, const double yPos,
               const bool sel) : router(r), x(xPos), y(yPos), selected(sel) {
    }
...
protected:
    ...
    void draw_node(const Cairo::RefPtr<Cairo::Context> &cr, std::shared_ptr<RouterNode> node);

    void draw_edge(const Cairo::RefPtr<Cairo::Context> &cr, std::shared_ptr<RouterNode> node_s,
